...
% Keep the recurring blue footer and white logo, with compact slide numbering.
\setbeamertemplate{footline}{%
  ...
  \begin{beamercolorbox}[wd=\paperwidth,ht=0.29cm,dp=0.11cm,leftskip=0.46cm,rightskip=0.46cm]{footer}%
    ...
    {\fontsize{6.3}{7.4}\selectfont \insertframenumber\,/\,\inserttotalframenumber}%
  \end{beamercolorbox}%
}
  ...
  \begin{columns}[T,totalwidth=\paperwidth]
    \begin{column}{0.60\paperwidth}
      \begin{beamercolorbox}[wd=0.60\paperwidth,ht=\paperheight,dp=0cm,sep=0pt]{title panel}
        \vspace{0.62cm}
        \hspace{0.72cm}\includegraphics[width=2.55cm]{segula_white.png}
        \vspace{0.48cm}
        ...
        \hspace{0.72cm}\begin{minipage}{0.49\paperwidth}
          ...
          \vspace{0.22cm}
          \colorbox{white}{\parbox[c][0.52cm][c]{2.25cm}{\centering\includegraphics[width=1.72cm]{R.png}}}
        \end{minipage}
      \end{beamercolorbox}
    \end{column}
...
% -----------------------------------------------------------------------------
\begin{frame}{One loop, three validation environments}
  \eyebrow{Virtual, physical and embedded validation}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.31\textwidth}
      \includegraphics[width=\linewidth,height=0.19\textheight,keepaspectratio]{qlabs_map.png}\par
      \vspace{0.07cm}
      \stepnumber{01}\hspace{0.10cm}{\fontsize{10.7}{12.7}\selectfont\bfseries Virtual campaign}\par
      {\color{Muted}\fontsize{7.5}{9}\selectfont Numerical and QLabs runs repeat the same states and attacks with controlled ground truth.}
    \end{column}
    \begin{column}{0.31\textwidth}
      \includegraphics[width=\linewidth,height=0.19\textheight,keepaspectratio]{scale_model_track.jpg}\par
      \vspace{0.07cm}
      \stepnumber{02}\hspace{0.10cm}{\fontsize{10.7}{12.7}\selectfont\bfseries Physical loop}\par
      {\color{Muted}\fontsize{7.5}{9}\selectfont QCar/LIMO trials add real packet timing, sensor noise, scheduling and actuator response.}
    \end{column}
    \begin{column}{0.31\textwidth}
      \includegraphics[width=\linewidth,height=0.19\textheight,keepaspectratio]{1car_board_smaller_img.jpeg}\par
      \vspace{0.07cm}
      \stepnumber{03}\hspace{0.10cm}{\fontsize{10.7}{12.7}\selectfont\bfseries Embedded transfer}\par
      {\color{Muted}\fontsize{7.5}{9}\selectfont The research PCB reproduces the loop with RTOS timing, sensing, communication and logging.}
    \end{column}
  \end{columns}
  \vspace{0.14cm}
  {\color{BrandBlue}\rule{0.08cm}{0.55cm}}\hspace{0.12cm}%
  \begin{minipage}[b]{0.90\linewidth}
    {\fontsize{8.2}{9.8}\selectfont\bfseries The algorithm and attack scenarios stay comparable while platform realism increases.}
  \end{minipage}
  \sourcefoot{Thesis Chapter 6 methodology and final manuscript validation layers}
  \note{[Sources]\par
    \texttt{\detokenize{ch6/ch6_main.tex}}, Experimental methodology: from virtual validation to real trials and the QLabs experimental workflow.\par
    \texttt{\detokenize{ch5/TRust_final/manuscript_v7_submit.tex}}, aggregate attack-window performance and QCar/LIMO platform validation.\par
    Images: user-provided project assets \texttt{\detokenize{assets/qlabs_map.png}}, \texttt{\detokenize{assets/scale_model_track.jpg}} and \texttt{\detokenize{assets/1car_board_smaller_img.jpeg}}.}
\end{frame}
...
% -----------------------------------------------------------------------------
\begin{frame}{Research PCB vs commercial OBU}
  \eyebrow{Direct comparison}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.46\textwidth}
      \centering
      \includegraphics[width=0.78\linewidth,height=0.18\textheight,keepaspectratio]{pcb_photo.jpg}\par
      \vspace{0.05cm}
      {\color{BrandBlue}\fontsize{11.2}{13.2}\selectfont\bfseries Embedded trust demonstrator}\par
      \vspace{0.08cm}
      {\raggedright\fontsize{7.6}{9.1}\selectfont
        \textbf{Core role:} state estimation, source trust, rollback and exported health flags.\par
        \vspace{0.07cm}
        \textbf{Interfaces:} IMU, GNSS/1PPS, research Wi-Fi, UART/USB, storage and synchronized logging.\par
        \vspace{0.07cm}
        \textbf{Readiness:} proves the embedded trust functions on a research vehicle platform.}
    \end{column}
    \begin{column}{0.49\textwidth}
      \centering
      \begin{minipage}[t]{0.47\linewidth}
        \centering
        \includegraphics[width=\linewidth,height=0.18\textheight,keepaspectratio]{cohda_mk6_obu.jpg}
      \end{minipage}\hfill
      \begin{minipage}[t]{0.47\linewidth}
        \centering
        \includegraphics[width=\linewidth,height=0.18\textheight,keepaspectratio]{commsignia_obu.jpg}
      \end{minipage}\par
      \vspace{0.05cm}
      {\color{BrandBlue}\fontsize{11.2}{13.2}\selectfont\bfseries Commercial OBU envelope}\par
      \vspace{0.08cm}
      {\raggedright\fontsize{7.6}{9.1}\selectfont
        \textbf{Core role:} deployment-ready V2X communication, positioning and vehicle integration.\par
        \vspace{0.07cm}
        \textbf{Interfaces:} DSRC/C-V2X, cellular/WLAN options, GNSS, CAN/GPIO and onboard sensor inputs.\par
        \vspace{0.07cm}
        \textbf{Readiness:} hardware-backed security, automotive design, certifications and lifecycle support.}
    \end{column}
  \end{columns}
  \vspace{0.06cm}
  {\color{BrandMagenta}\rule{0.08cm}{0.55cm}}\hspace{0.12cm}%
  \begin{minipage}[b]{0.90\linewidth}
    {\fontsize{7.6}{9.1}\selectfont\bfseries Next step:} {\fontsize{7.6}{9.1}\selectfont keep the trust layer; add vehicle I/O, production V2X, secure hardware and qualification.}
  \end{minipage}
  \sourcefoot{Thesis Chapter 6; official Cohda Wireless MK6 and Commsignia OBU product pages}
  \note{[Sources]\par
    Image: user-provided project asset \texttt{\detokenize{assets/pcb_photo.jpg}}.\par
    \texttt{\detokenize{ch6/ch6_main.tex}}, board motivation, functional requirements, component selection, firmware structure and integration roadmap.\par

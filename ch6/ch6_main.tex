...
That also suupport the lateral control loop by providing a reference heading and curvature that the look-ahead controller can use to compute steering commands.
The MPC optimization problem is solved at each control step, using the current state estimate and the reference
...
Although deploying software directly on a vehicle computer may be acceptable for a single dedicated platform, this approach becomes impractical when scaling up to test multiple vehicles or when working across different testbeds: the software environment is strongly vehicle-dependent, access to low-level signals and time bases can be constrained, and modifications to each onboard stack increase integration time, maintenance effort, and safety risk.
The proposed embedded electronic board addresses these issues by acting as a self-contained module that can be integrated as an add-on to different platforms without deeply modifying the vehicle control architecture.

In addition, the board is designed as a \emph{portable ecosystem}: it provides a stable hardware/software baseline (drivers, timestamping, message formats, diagnostics, logging) so that the same implementation can be reused across multiple testbeds (scaled vehicles such as QCar, experimental prototypes, and future full-scale platforms).
This portability reduces the cost of transferring algorithms between platforms, improves experiment repeatability, and supports progressive deployment (monitoring-only mode first, then tighter coupling with control once validated).

In this thesis, the board is therefore envisioned as an embedded \emph{intelligent soft sensor}: it reconstructs non-measured variables, monitors the consistency of cooperative data, and exports reliable state and trust indicators to higher-level decision and control.
This rationale is also driven by practical deployment constraints: in connected and autonomous vehicles, simultaneously measuring all the variables required by cooperative control and diagnosis (distances, relative velocities/accelerations, attitude/heading, road adhesion proxies, etc.) is often costly, and some quantities remain inaccessible with the available instrumentation.
Model-based observers can compensate for missing measurements by reconstructing critical states online, but their effectiveness depends on deterministic timing, consistent timestamping, and reliable access to both local sensor data and cooperative messages.
The VEHALSECU R\&D effort therefore targets a compact onboard module able to perform state estimation and cyberattack-aware consistency monitoring even under reduced sensing and intermittent/corrupted V2V communication~\cite{chaix2026vehalsecu}.

From a system engineering viewpoint, the board design addresses three main challenges: (i) ensuring deterministic real-time behavior for observer and control tasks (e.g., $\approx 100\,\mathrm{Hz}$ loops with bounded jitter), (ii) integrating multiple sensor modalities and communication interfaces while maintaining time consistency across all data streams, and (iii) supporting deployment across different vehicle platforms through standardized interfaces and a portable software baseline.

\paragraph{Functional requirements.}
To meet these objectives, the needs analysis identifies the following embedded capabilities required for onboard deployment:
\begin{itemize}
	\item \textbf{Inertial sensing:} an Inertial Measurement Unit (IMU) combined with a magnetometer to measure acceleration, angular rates, and heading;
	\item \textbf{Positioning and time reference:} a Global Navigation Satellite System (GNSS) receiver to provide absolute position and a synchronized time base;
	\item \textbf{Data logging:} removable storage (e.g., microSD card) to record sensor data, received messages, and estimation outputs for offline analysis;
	\item \textbf{Wired diagnostics interface:} Universal Serial Bus (USB) and Universal Asynchronous Receiver-Transmitter (UART) connections for firmware programming, debugging, and telemetry;
	\item \textbf{Wireless communication:} a short-range wireless interface (Wi-Fi) to support Vehicle-to-Vehicle (V2V) data exchange;
	\item \textbf{Power management:} a regulated power stage capable of converting the nominal vehicle supply voltage ($12\,\mathrm{V}$) into stable rails for digital and RF components.
\end{itemize}
This modular architecture ensures that the board can operate independently while interfacing with different vehicle platforms~\cite{chaix2026vehalsecu}.

From a vehicle-integration perspective, this embedded demonstrator must also remain robust to typical onboard constraints, including supply transients, Electrostatic Discharge (ESD) events on external cabling, mechanical vibration, and Electromagnetic Interference (EMI) generated by actuators and switching power stages.
These constraints directly influence the PCB stack-up and placement (return-path control and RF keep-outs), connector and protection choices, and firmware safety mechanisms such as watchdog supervision and brown-out handling.

Finally, the move to embedded hardware opens the door to security-by-design evaluations that are difficult to emulate in a PC-only setup (device identity, secure storage, authenticated firmware updates, and message authentication).
While the present prototype focuses on functionality and timing, the architecture is intended to remain compatible with later upgrades toward authenticated communication and secure boot.
...
To keep the embedded implementation compatible with the thesis algorithms, the architecture definition includes a first-order sizing step: expected sensor sampling rates and message rates are translated into data throughput and storage requirements, which then guide the selection of memory resources and logging mechanisms~\cite{chaix2026vehalsecu}.
In practice, this motivates (i) separating time-critical sensor acquisition from less deterministic communication bursts and (ii) providing an onboard storage path to retain raw measurements and intermediate signals for offline analysis.
...
For the first experimental prototype deployed on a scaled test platform, Wi-Fi~6 was retained because it is comparatively accessible to implement while still providing sufficient performance for short-range V2V exchanges; this choice also simplifies integration and experimentation before migrating to more automotive-oriented stacks if required~\cite{chaix2026vehalsecu}.
...
The initial validation protocols described in~\cite{chaix2026vehalsecu} confirm:
\begin{itemize}
...
First, power integrity is validated through visual inspection and continuity checks, then by a progressive power-up with current limiting; regulated rails are verified under no-load conditions and their ripple is measured with an oscilloscope to detect unstable regulators or layout-related issues.
Second, the communication interfaces are validated with simple, repeatable transactions: UART loopback/telemetry, I$^2$C bus scan and register reads, SPI transfers to a known peripheral (or a synthetic test pattern), and finally GNSS acquisition and Wi-Fi association followed by basic data transfer tests (IP assignment, ping, packet sending)~\cite{chaix2026vehalsecu}.

In addition to software-level checks, bench bring-up measurements provide quantitative confidence in the electrical design.
Reported results include a regulated $3.3\,\mathrm{V}$ rail close to nominal with limited ripple, stable bidirectional UART communication (e.g., $115200$\,baud), GNSS fix acquisition within tens of seconds with meter-level horizontal accuracy, and a functional Wi-Fi~6 link with measured throughput on the order of tens of Mbit/s in a test setup~\cite{chaix2026vehalsecu}.

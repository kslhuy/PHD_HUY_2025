...
Since $A(v_x)$ depends on both $v_x$ and $1/v_x$, we define the scheduling variables
\[
\rho_1(t):=v_x(t), \qquad \rho_2(t):=\frac{1}{v_x(t)}.
\]
For $v_x(t)\in[\underline v_x,\overline v_x]$, the matrix is embedded into the polytopic set
\[
A(v_x(t))\in\mathrm{Co}\{A_1,\dots,A_{\bar n}\},
\]
where the vertices $A_j$ are obtained by evaluating $A(v_x)$ at the corners of $(\rho_1,\rho_2)$ (typically $\bar n=4$). The observer gains are synthesized by enforcing the LMI conditions at all vertices and using a common (constant) gain.

\subsubsection{UIO for the LPV Vehicle Model (Gain $K$)}
The teacher component is the generalized/regularized UIO of Section~\ref{uio}, which estimates the augmented variable $\zeta=[x^\top,\mu_x^\top]^\top$. To satisfy the augmented rank condition \eqref{rank2}, we use the regularized output
\begin{equation}
    y = Cx + D_{\delta_2}\mu_x + D\,\omega_{\delta_2},
\end{equation}
with $D_{\delta_2}=\delta_2D$ for a small $\delta_2>0$ and a binary matrix $D$ such that $\mathrm{rank}(D)=n_\mu$. For $n_y=3$ and $n_\mu=2$, a convenient choice is
\begin{equation}
    D=\begin{bmatrix}0&0\\1&0\\0&1\end{bmatrix}.
\end{equation}
The observer dynamics are written as
\begin{equation}
\left\{
\begin{array}{rcl}
\dot{\eta} & = & P_{\zeta}\varphi_{\zeta}(\hat{\zeta},u)+K\left(y-\mathcal{H}\hat{\zeta}\right) \\
...
\right.
\end{equation}
where $P_{\zeta}$ and $Q_{\zeta}$ satisfy \eqref{ali_syst2_e2}. The polytopic Jacobian vertices are
\begin{equation}
\mathcal{A}_{\zeta,j}=
\begin{bmatrix}
A_j & E \\
...
\end{equation}
and the gain is computed from Theorem~\ref{ali_thm} as $K=\mathcal{P}^{-1}\mathcal{Q}^\top$. In the Teacher--Student architecture, the UIO provides a robust reference estimate (in particular, $\hat{\mu}_x$) that is used as a supervisory signal in the neural learning loss.
...
\begin{equation}
    \dot{\hat{x}}_{NN} = A(v_x)\hat{x}_{NN} + Bu + E\mu_\theta(\hat{x}_{NN}) + L_{NN}\big(y-C\hat{x}_{NN}\big).
\end{equation}
The gain $L_{NN}$ is designed using Theorem~\ref{ali_thm2} over the vertex set $\{A_j\}_{j=1}^{\bar n}$, yielding $L_{NN}=\mathcal{P}^{-1}\mathcal{Q}^\top$.

\subsubsection{Gradient Computation Using the Vehicle LPV Model}
For the online update of $\theta$, the NAO is discretized with Euler step $\Delta t$:
\[
\hat{x}_{k+1} = \hat{x}_k + \Delta t\,f_{\mathrm{obs}}(\hat{x}_k,\theta,\rho_k), \qquad \rho_k=v_{x,k}=y_{1,k}.
\]
The sensitivity recursion used to compute $\nabla_\theta\mathcal{L}$ is
\begin{equation}
S_{k+1}=S_k+\Delta t\left(A_kS_k+B_k\right), \qquad S_k:=\frac{\partial \hat{x}_k}{\partial \theta},
\end{equation}
with
\begin{align}
...
\end{align}
This recursion highlights that learning remains explicitly anchored to the physical LPV structure through $A(\rho_k)$ at each sampling instant.

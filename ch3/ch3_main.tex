...
The CACC scenario working on a platoon can be illustrated as in Figure~\ref{fig_pre_fol}.
 ...
 \begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{ch3/img/SACC.png}
    \caption{Illustration of CACC working on a vehicle platoon.}
    \label{fig_pre_fol}
\end{figure} 
 %=====
The acceleration of preceding vehicle $a_{i-1}$ transmitted through a wireless communication channel to the following vehicle; $\delta_{i}$ denotes the distance between the rear of the preceding vehicle and the bumper of the following vehicle; $l_{i}$ is the length of the following vehicle; $p_{i}$ and $p_{i-1}$ correspond to the positions of the following and preceding vehicles, respectively.

Following the logic of ~\cite{rajamani2002semi}, The CACC control system focuses only on the upper-level controller for acceleration input. It assumes the lower-level execution of throttle and brake control is already functional and optimized
...
\begin{equation}\label{control_uc}
...
%===== Figure ================================================
 \begin{figure}[h!]
     \centering    
        \includegraphics[width=1\textwidth]{ch3/img/fig_discrete-time.png}
...
\begin{equation}\label{CACC_discrete_delay}
      \left\{ \begin{array}{l}
        z_{t+1} = \mathcal{A} z_{t}+\mathcal{B}  v_{i,t} + \mathcal{F}  \mu_{t} + \bar{\Delta} - \mathcal{W}  f^c_{t} + E_{\omega} \omega_{t}\\
...
\tilde{\xi}_{t+1} &=  \Big(P_z A_{\xi}-K C_{\xi}\Big) \tilde{\xi}_{t} + \begin{bmatrix}\Big(K D_{\omega} - P_z E_{\omega}\Big) & Q_z D_{\omega} \end{bmatrix} \bar{\omega}_{t},
\end{align}
...
\begin{empheq}[box = \widebox]{gather}\label{ali_eISS}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{ch3/img/bla_bla.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\linewidth]{ch3/img/matlab_error_case_2.eps}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\linewidth]{ch3/img/matlab_fc_2.eps}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\linewidth]{ch3/img/matlab_fc_1.eps}
...
%  \begin{figure}[h!]
%      \centering    
%      \includegraphics[width=0.6\textwidth,height = 3.5cm]{ch3/img/carla_d.png}
...
% \begin{figure}[h!]
%     \centering    
%     \includegraphics[scale=1]{ch3/img/state_carla_new.eps}
%     \caption{State estimation using CARLA under attack case 2.}
%     \label{fig:carla_state}
% \end{figure}
% \begin{figure}[h!]
%     \centering    \includegraphics[scale=0.9]{ch3/img/fc_carla_new.eps}
...
The observer design developed in the previous sections relies on a discrete-time model obtained via forward Euler discretization. This choice is convenient because it preserves the continuous-time structure and enables the direct use of classical UIO existence and stability results. However, forward Euler may destroy the algebraic coupling between the unknown input and the measured output, which can lead to a violation of the rank condition and, consequently, to the use of delayed outputs. Importantly, this delay is not a physical requirement of the CACC dynamics; it is a numerical artifact induced by the discretization method.

The key intuition is simple: a force (or acceleration) affects the vehicle motion \emph{within} each sampling interval. In particular, acceleration influences velocity and position through higher-order terms (e.g., $\tfrac{1}{2}aT_s^2$). By contrast, the Euler update $p_{k+1}=p_k+T_s v_k$ neglects this intra-sample coupling, which can sever the direct link needed by the rank condition. To preserve these higher-order effects, we adopt an exact (ZOH) discretization based on the matrix exponential~\cite{M.Zhang}. The resulting discrete-time model captures the full state evolution between samples and restores the structural coupling required for a delay-free UIO.

Figure~\ref{fig_dis_UIO} illustrates the observer-based cyberattack estimation framework using the exact discretization model.

\begin{figure}[h!]
\centering
\includegraphics[width=1\textwidth]{ch3/img/probleme_2.png}
\caption{Discrete-time model-based UIO}
\label{fig_dis_UIO}
\end{figure}
...
%--
To evaluate the impact of the discretization method, we compare two discrete-time designs built from the same continuous-time model: (i) the Euler-discretized (delayed-output) observer presented in the previous sections and (ii) the proposed delay-free observer based on exact (ZOH) discretization. We conduct CARLA simulations following the general procedures described in~\cite{nguyen2020cyberattack}.
...
\begin{equation*}
    K_{euler} =\begin{bmatrix}
        0.2500 & -0.0025 & 0.2500 & -11.1139 \\
        -0.0050 & 1.0000 & -99.9950 & 1851.0714 
      \end{bmatrix}^{\top}.
\end{equation*} 

\begin{equation*}
    K_{exact} =\begin{bmatrix}
        0.2500 & 0.0012 &  -0.3080 & 6.6744 \\
        -5.3\times10^{-5} & 0.4461 & -46.1776 & 1216.7071 
      \end{bmatrix}^{\top}.
\end{equation*} 

\begin{figure}[h!]
    \centering 
    \includegraphics[width=1\linewidth]{ch3/img/vehicle_trajectoies.eps}
    \caption{Vehicle trajectories of preceding-following vehicle.}
    \label{vehicle_trajectories_fig}
\end{figure}

\begin{figure}[h!]
    \centering 
    \includegraphics[width=1\linewidth]{ch3/img/control_input.eps}
    \caption{Control input of preceding-following vehicle.}
    \label{control_input_fig}
\end{figure}

Figure~\ref{control_input_fig} indicates that the discretization method also impacts the control effort. In particular, the exact discretization leads to a less aggressive input. Table~\ref{tab:ch3_control_input_metrics} confirms this trend by showing reductions in both RMS and peak values of $u_{\text{sync}}$ after the warm-up period.

\begin{table}[h!]
\centering
\begin{tabular}{|c|c|c|}
\hline
\textbf{Metric} & \textbf{Euler} & \textbf{Exact (ZOH)} \\ \hline
RMS (after warm-up)  & 4.609962 & 3.633092 \\ \hline
Peak (after warm-up) & 13.487141 & 9.389372 \\ \hline
\end{tabular}
\caption{Control input metrics $u_{\text{sync}}$ for Euler and exact (ZOH) discretization (CARLA simulation).}
\label{tab:ch3_control_input_metrics}
\end{table}


\begin{figure}[h!]
    \centering 
    \includegraphics[width=1\linewidth]{ch3/img/eror_state_estimation.eps}
    \caption{State estimation error of 2 discretization method.}
    \label{est_error_sel_fig}
\end{figure}

Figure~\ref{est_error_sel_fig} compares the state estimation errors for position, velocity, and acceleration under Euler and exact discretization. Overall, the exact discretization approach yields a faster transient response and visibly reduces oscillations and noise, with the most pronounced improvement observed for the acceleration estimate.

To quantify this observation, Table~\ref{tab:ch3_state_estimation_errors} reports the RMS and MSE values for both discretization methods. While the position error is similar in both cases, the exact discretization reduces the velocity RMS by about $4.5\%$ and the acceleration RMS by about $40\%$. A similar trend is observed in MSE, where the acceleration MSE decreases by more than $60\%$. These results support the claim that preserving the exact inter-sample dynamics improves both accuracy and robustness.

\begin{table}[h!]
\centering
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Metric} & \textbf{Position} & \textbf{Velocity} & \textbf{Acceleration} \\ \hline
RMS (Euler)  & 0.35637 & 0.20425 & 1.508381 \\ \hline
RMS (Exact)  & 0.35600 & 0.19503 & 0.912209 \\ \hline
MSE (Euler)  & 0.01270 & 0.0417 & 2.275214 \\ \hline
MSE (Exact)  & 0.01267 & 0.0380 & 0.832126 \\ \hline
\end{tabular}
\caption{State estimation error metrics for Euler and exact discretization (CARLA simulation).}
\label{tab:ch3_state_estimation_errors}
\end{table}

\begin{figure}[h!]
    \centering 
    \includegraphics[width=1\linewidth]{ch3/img/compare_est_f_c.eps}
    \caption{Comparison of cyberattack estimation and error.}
    \label{fig:attack_est}
\end{figure}

Figure~\ref{fig:attack_est} shows the true cyberattack signal and its reconstruction, along with the estimation error. Both discretization strategies enable attack detection; however, the exact discretization yields a smoother reconstruction and a smaller estimation error, which is consistent with the improved state estimates reported above.
